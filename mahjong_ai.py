...
def make_model():
    ...
    #1層目の隠れ層
    w.append(tf.Variable(tf.random.truncated_normal([NUM_HAI, H_SIZE], stddev=0.1)))
    ...
    for i in range(1, NUM_HIDDEN_LAYERS):
        w.append(tf.Variable(tf.random.truncated_normal([H_SIZE, H_SIZE], stddev=0.1)))
        b.append(tf.Variable(tf.zeros([H_SIZE])))
        h.append(tf.nn.sigmoid(tf.matmul(h[i-1], w[i]) + b[i]))
        #h.append(tf.nn.relu(tf.matmul(h[i-1], w[i]) + b[i]))

    #出力層
    w.append(tf.Variable(tf.random.truncated_normal([H_SIZE, NUM_HAI], stddev=0.1)))
    ...
    #正解データの型を定義
    y = tf.compat.v1.compat.v1.placeholder(tf.float32, [None, NUM_HAI])
    #誤差関数（クロスエントロピー）
